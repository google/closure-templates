...
/**
 * A class for building Java code.
 *
 * TODO: fix doc
 * <p> Usage example that demonstrates most of the methods:
 * <pre>
 *   JavaCodeBuilder jcb = new JavaCodeBuilder(CodeStyle.STRINGBUILDER);
 ...
 *       new JavaExpr("'Snow White and the '", Integer.MAX_VALUE),
 *       new JavaExpr("opt_data.numDwarfs", Integer.MAX_VALUE));
 *   jcb.popOutputVar();
 *   jcb.addToOutputVar(Lists.newArrayList(
 *       new JavaExpr("temp", Integer.MAX_VALUE),
 *       new JavaExpr("' Dwarfs'", Integer.MAX_VALUE));
 ...
 * </pre>
 * The above example builds the following Java code:
 * <pre>
...
class JavaCodeBuilder {
   ...
   */
  public JavaCodeBuilder(CodeStyle codeStyle) {
  ...
  /**
   * Tells this JavaCodeBuilder that the current output variable has already been initialized. This
   ...
   * @return This JavaCodeBuilder (for stringing together operations).
   */
  ...
  /**
   * Equvalent to javaCodeBuilder.indent().append(javaCodeFragments).append("\n");
   * @param javaCodeFragments The code string(s) to append.
   * @return This JavaCodeBuilder (for stringing together operations).
   */
  public JavaCodeBuilder appendLine(String... javaCodeFragments) {
    indent();
    append(javaCodeFragments);
    code.append("\n");
    return this;
  }
  ...
  public void initOutputVarIfNecessary() {
    ...
    if (codeStyle == CodeStyle.STRINGBUILDER) {
      ...
      appendLine("StringBuilder ", currOutputVarName, " = new StringBuilder();");
    } else {
      // String output = "";
      appendLine("String ", currOutputVarName, " = \"\";");
    }
    setOutputVarInited();
  }
  ...
  /**
   * Appends a line/statement with the concatenation of the given Java expressions saved to the
   * current output variable.
   * @param javaExprs One or more Java expressions to compute output.
   */
  public void addToOutputVar(List<JavaExpr> javaExprs) {

    if (codeStyle == CodeStyle.STRINGBUILDER) {
      // TODO: Clean this up. No need for joinedJavaExprsSb.
      ...
      if (currOutputVarIsInited) {
        // output.append(AAA).append(BBB);
        appendLine(currOutputVarName, ".append(", joinedJavaExprsSb.toString(), ");");
      } else {
        // StringBuilder output = (new StringBuilder()).append(AAA).append(BBB);
        appendLine("StringBuilder ", currOutputVarName, " = (new StringBuilder()).append(",
