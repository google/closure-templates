...
public final class SharedRuntime {
  ...
  private static boolean isNullishNumber(SoyValue value) {
    return value instanceof NumberData || value.isNullish();
  }

  /** Performs the {@code +} operator on the two values. */
  @Nonnull
  public static SoyValue plus(SoyValue operand0, SoyValue operand1) {
    if (isNullishNumber(operand0) && isNullishNumber(operand1)) {
      return NumberData.forValue(toDoubleForNumericOp(operand0) + toDoubleForNumericOp(operand1));
    } else {
  ...
  public static NumberData negative(SoyValue node) {
    return NumberData.forValue(node instanceof NullData ? 0 : -node.floatValue());
  }
