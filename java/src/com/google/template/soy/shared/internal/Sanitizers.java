...
public final class Sanitizers {
              ...
              if (isClose) {
                if (openTags.get() != null) {
                  ...
                  if (lastIdx >= 0) {
                    ...
                    // If we didn't, then we would convert "<ul><li></ul>" to
                    // "<ul><li></ul></li>"
                    // which could lead to broken layout for embedding HTML that uses lists
                    // for
                    // formatting.
                    // This leads to observably different behavior for adoption-agency
                    // dependent
                ...
                if (openListTagCount.get() > 0 || !"li".equals(tagName)) {
                  ...
                  // Most attributes are dropped, but the dir attribute is preserved if it
                  // exists.
                  // The attribute matching could be made more generic if more attributes need
                  // to be
                  // whitelisted in the future. There are also probably other utilities in
                  // common to
  ...
  private static CharSequence replaceHtmlTags(
      String s, BiFunction<String, String, String> callback, Function<String, String> escaper) {
      ...
      switch (state) {
        case DEFAULT:
          ...
          if (nextLt < 0) {
            ...
            if (buffer.length() == 0) {
              return escaper.apply(s);
            }
            buffer.append(escaper.apply(s.substring(i)));
            i = l;
          } else {
            // Push up to < onto buffer.
            buffer.append(escaper.apply(s.substring(i, nextLt)));
            tagStartIdx = nextLt;
            i = nextLt + 1;
            // Search for required token after <
            Matcher m = HTML_TAG_FIRST_TOKEN.matcher(s);
