 ...
 *
 * <p> This functions similarly to string.intern(). It has extremely low memory overhead of a
...
final class CompactInterner {
   ...
   *
   * <p> Increasing this number reduces memory usage at the expense of more equals() checks
   * for both cache hits and misses. The max load factor of the table is
   * 1 / (MAX_EXPECTED_COLLISION_COUNT + 1).
   */
   ...
   *
   * <p> Increasing this number results in better memory utilization at the expense of more
   * frequent rehashing.
   *
  ...
  @VisibleForTesting double getAverageCollisions() {
    return 1.0 * collisions / count;
  }


  @VisibleForTesting static double getAverageCollisionsBound() {
  ...
  @VisibleForTesting double getOverhead() {
    return 1.0 * (table.length - count) / count;
  }


  @VisibleForTesting static final double getWorstCaseOverhead() {
