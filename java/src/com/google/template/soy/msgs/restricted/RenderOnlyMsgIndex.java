...
public final class RenderOnlyMsgIndex {
  ...
  public RenderOnlyMsgIndex(ImmutableList<Long> allIds) {
    this.table = buildTable(new ArrayList<>(allIds));
    this.size = allIds.size();
  }
  ...
  private static int tableSize(int newSize) {
  ...
  Accessor buildAccessor(ImmutableList<RenderOnlySoyMsgBundleImpl.RenderOnlySoyMsg> msgs) {
    ...
    synchronized (this) {
      ...
      checkState(allIds.size() == newSize);
  ...
  private static long[] buildTable(List<Long> allIds) {
    ...
    allIds.sort(
        comparingInt((Long id) -> hashIndex(id, newTableMask))
            .thenComparingLong((Long id) -> id.longValue()));
  ...
  private static Accessor makeAccessor(
      ...
      int[] indices) {
    Object[] values = new Object[Arrays.stream(indices).max().orElse(-1) + 1];
    for (int i = 0; i < msgs.size(); i++) {
