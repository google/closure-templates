 ...
 * <p>
 * Thus, in a single call, the formatter can escape the input string as specified, determine its
...
public class BidiFormatter {
   ...
   * surroundings.
   * <p>
   * The algorithm: estimates the directionality of the given string. In case its directionality
   * doesn't match the context directionality, wraps it with a 'span' element and adds a "dir"
   * attribute (either 'dir=\"rtl\"' or 'dir=\"ltr\"').
   * <p>
   * If the formatter was built using {@link #alwaysSpan(true)}, the input is always wrapped
   ...
   * <p>
   * If !{@code isHtml}, HTML-escapes the string regardless of wrapping.
   *
  ...
  /**
   * Operates like {@link #spanWrap(String, boolean, boolean)}, but assumes {@code isolate} is
   * true.
   *
   * @param str The input string
   * @param isHtml Whether {@code str} is HTML / HTML-escaped
   ...
   * the string with a 'span' element and adds a 'dir' attribute (either 'dir=\"rtl\"' or
   * 'dir=\"ltr\"').
   * <p>
   * If the formatter was built using {@link #alwaysSpan(true)}, the input is always wrapped
   * in a span, skipping just the dir attribute when it's not needed.
  ...
  /**
   * Operates like {@link #spanWrapWithKnownDir(Dir, String, boolean, boolean)}, but
  ...
  /**
   * Operates like {@link #spanWrapWithKnownDir(Dir, String, boolean, boolean)}, but
   * assumes {@code isHtml} is false and {@code isolate} is true.
   *
   * @param dir {@code str}'s directionality
   * @param str The input string
   * @return Input string after applying the above processing.
  ...
  /**
   * Operates like {@link #unicodeWrapWithKnownDir(Dir, String, boolean, boolean)}, but
   * assumes {@code isolate} is true.
   *
   * @param dir {@code str}'s directionality
   * @param str The input string
  ...
  public String markAfterKnownDir(@Nullable Dir dir, String str, boolean isHtml) {
    ...
    if (contextDir == Dir.LTR &&
        (dir == Dir.RTL || BidiUtils.getExitDir(str, isHtml) == Dir.RTL)) {
      return BidiUtils.Format.LRM_STRING;
    }
    if (contextDir == Dir.RTL &&
        (dir == Dir.LTR || BidiUtils.getExitDir(str, isHtml) == Dir.LTR)) {
  ...
  /**
   * Operates like {@link #markAfterKnownDir(Dir, String, boolean)}, but assumes that
  ...
  /**
   * Operates like {@link #markBeforeKnownDir(Dir, String, boolean)}, but assumes that
   * {@code isHtml} is false.
   *
   * @param str String before which the mark may need to appear
   * @param dir {@code str}'s overall directionality
   ...
   */
  public String mark() {
    return contextDir == Dir.LTR ? BidiUtils.Format.LRM_STRING :
