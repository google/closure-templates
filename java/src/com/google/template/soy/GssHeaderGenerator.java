...
/**
 * Entry point for generating GSS header files corresponding to Soy files.
 *
 * <p>Important: Do not use outside of Soy code (treat as superpackage-private).
 */
public final class GssHeaderGenerator {

  public GssHeaderGenerator() {}

  /** Append import statements for the generated Gss header files for all dependencies */
  private static void addCodeToImportGeneratedHeaders(StringBuilder output, SoyFileNode soyFile) {
    output.append("\n");
    output.append("/* Import the generated Gss header files for all dependencies */\n");
    soyFile.getImports().stream()
        .filter(i -> i.getImportType() == ImportType.TEMPLATE)
        .map(i -> i.getSourceFilePath().path())
        .distinct()
        .sorted()
        .forEach(soyImport -> output.append("@require('").append(soyImport).append("');\n"));
  }
  ...
  private static void addCodeToRequireCss(StringBuilder output, SoyFileNode soyFile) {
    ...
    soyFile.getRequireCss().stream()
        .distinct()
        .sorted()
        .forEach(gssImport -> output.append("@require('").append(gssImport).append("');\n"));
  }

  public List<String> genGssHeaders(SoyFileSetNode fileSetNode) {
