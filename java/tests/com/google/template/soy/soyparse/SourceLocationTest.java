...
public final class SourceLocationTest {
  ...
  private static final SourceFilePath FAKE_FILE_PATH = SourceFilePath.forTest("fakefile.soy");
  ...
  public void testForLoop() throws Exception {
    assertSourceRanges(
        JOINER.join(
            "SoyFileNode",
            "  TemplateBasicNode            {template foo}Hello{for $[...]lanet}{/for}!{/template}",
            "    RawTextNode                Hello",
            "    ForNode                    {for $planet in ['mercury[...]]},{print $planet}{/for}",
  ...
  public void testExpressions() throws Exception {
    assertSourceRanges(
        JOINER.join(
            ...
            "              VarRefNode       $b",
            "          VarRefNode           $c",
            "    PrintNode                  {$l[$a * $b] < $c && !$l?[...] ($b ? length($l) : $c)}",
            "      ExprRootNode             $l[$a * $b] < $c && !$l?[$c] ?? ($b ? length($l) : $c)",
            "        NullCoalescingOpNode   $l[$a * $b] < $c && !$l?[$c] ?? ($b ? length($l) : $c)",
            "          AmpAmpOpNode         $l[$a * $b] < $c && !$l?[$c]",
        ...
        JOINER.join(
            ...
            "  {@param b: int}",
            "  {@param c: int}",
            "  {@param l: list<float|int> = [1, 2, 3*4]}",
  ...
  public void testTrailingCommentsInNonClosingNodes() {
    assertSourceRanges(
        JOINER.join(
            ...
            "    ForNode                    {for $foo in $foolist}{if[...]is co...{/if}// C3{/for}",
