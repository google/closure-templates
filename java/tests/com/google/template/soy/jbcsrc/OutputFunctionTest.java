...
/** Tests for {@code {outputfunction}} compilation and runtime execution in jbcsrc. */
...
public final class OutputFunctionTest {
  ...
  private static final String SOY_FILE =
      ...
      {outputfunction testString: (s: string) => string}
      ...
      {outputfunction testHtml: (s: string) => html}
        {javaimpl class="com.google.template.soy.jbcsrc.OutputFunctionTest"
      ...
      {template calleeHtml}
        {@param fn: outputfunction (s: string) => html}
        {$fn('pointer')}
      {/template}
      ...
      {template calleeHtmlCoerced}
        {@param fn: outputfunction (s: string) => html}
        {'' + $fn('pointer')}
      {/template}

      {template callerHtmlCoerced}
        {call calleeHtmlCoerced}
          {param fn: testHtml /}
        {/call}
      {/template}

      {template calleeString}
        {@param fn: outputfunction (s: string) => string}
        {$fn('pointer')}
      {/template}

      {template callerString}
        {call calleeString}
          {param fn: testString /}
        {/call}
      {/template}

      {template calleeStringCoerced}
        {@param fn: outputfunction (s: string) => string}
        {$fn('pointer') + '!'}
      {/template}

      {template callerStringCoerced}
        {call calleeStringCoerced}
          {param fn: testString /}
        {/call}
      {/template}
      """;
  ...
  @Test
  public void testFunctionPointerHtmlDirect() throws Exception {
    assertThat(render("test.callerHtml")).isEqualTo("primary: pointer");
  }

  @Test
  public void testFunctionPointerHtmlCoerced() throws Exception {
    assertThat(render("test.callerHtmlCoerced")).isEqualTo("fallback: pointer");
  }

  @Test
  public void testFunctionPointerStringDirect() throws Exception {
    assertThat(render("test.callerString")).isEqualTo("primary: pointer");
  }

  @Test
  public void testFunctionPointerStringCoerced() throws Exception {
    assertThat(render("test.callerStringCoerced")).isEqualTo("fallback: pointer!");
  }

  @Test
  public void testOutputFunctionNotAssignableToStandardFunction() {
    String file =
        """
        {namespace test}
        {outputfunction testHtml: (s: string) => html}
        ...
        {/outputfunction}
        {template callee}
          {@param fn: (s: string) => html}
          {$fn('a')}
        {/template}
        {template caller}
          {call callee}{param fn: testHtml /}{/call}
        {/template}
        """;
    AssertionError e = assertThrows(AssertionError.class, () -> TemplateTester.compileFile(file));
    assertThat(e).hasMessageThat().contains("Type mismatch on param fn");
    assertThat(e).hasMessageThat().contains("expected: (s: string) => html");
    assertThat(e).hasMessageThat().contains("actual: outputfunction (s: string) => html");
  }
}
