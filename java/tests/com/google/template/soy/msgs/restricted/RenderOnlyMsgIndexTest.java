...
public final class RenderOnlyMsgIndexTest {
  ...
  // Regression test for a bug where if the RenderOnlyMsgIndex table was allocated with an empty
  // list of ids we would create an internal array of size 1 which would cause us to compute bad
  // `masks` leading to ArrayIndexOutOfBoundExceptions.
  @Test
  public void testAIOOBE() {
    long id = 2369742814572661392L;
    assertThat(
            new RenderOnlyMsgIndex(ImmutableList.of())
                .buildAccessor(ImmutableList.of())
                .getBasicTranslation(id))
        .isNull();
  }

  @Test
  public void testTableSize() {
    assertThrows(IllegalArgumentException.class, () -> RenderOnlyMsgIndex.tableSize(0));
    assertThat(RenderOnlyMsgIndex.tableSize(1)).isEqualTo(4);
    assertThat(RenderOnlyMsgIndex.tableSize(2)).isEqualTo(8);
    assertThat(RenderOnlyMsgIndex.tableSize(3)).isEqualTo(16);
    assertThat(RenderOnlyMsgIndex.tableSize(4)).isEqualTo(16);
    assertThat(RenderOnlyMsgIndex.tableSize(5)).isEqualTo(32);
  }
}
