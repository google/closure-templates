...
### `map.clear()` {#mutable_map-any,any_clear}

Empties the map in place.

### `map.delete(key)` {#mutable_map-any,any_delete}

Removes the value for a given key, in place.

Returns `true` if an entry was removed, `false` if the key was not found in the
map.
