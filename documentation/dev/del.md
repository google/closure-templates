...
If you put your default `deltemplate` in one file and the callee in another it
is likely that AJD (go/ajd) will prune the `deltemplate` implementation. The
best practice workaround for this is to use a wrapper template. For example,
...
```soy {.bad}
{deltemplate foo.bar.Baz}
  ...
{/deltemplate}
```

In this scenario, `b.soy` will tend to get pruned by AJD since there is no
`goog.require` linking the two files.

To resolve this you can rewrite like this:
...
This way `a.soy` will `goog.require` `b.soy` and the deltemplate won't get
pruned.
