...
records                | Return: `? extends java.util.Map`, `com.google.template.soy.data.SoyRecord`<br/> Param: `java.util.Map`, `com.google.common.collect.ImmutableMap`, `com.google.template.soy.data.SoyRecord` | `null` values provided as Java `null` if the Java map type allows null values, otherwise as `NullData`. `undefined` values provided as `UndefinedData`.
