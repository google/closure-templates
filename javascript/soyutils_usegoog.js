  ...
  for (let i = 0; i < values.length; i++) {
    ...
    if (Array.isArray(v)) {
      for (let j = 0; j < v.length; j++) {
        if ($$isTruthyNonEmpty(v[j])) {
          result.push(v[j]);
        }
      }
    } else if ($$isTruthyNonEmpty(v)) {
      result.push(v);
    }
  }
  return result.join(';');
 ...
 * @return {!Function}
 */
const $$bindFunctionParams = function(f, params) {
 ...
 * @type {!RegExp}
 */
const $$FILTER_FOR_FILTER_NORMALIZE_MEDIA_URI_ = /^[^&:\/?#]*(?:[\/?#]|$)|^https?:|^filesystem:https?:|^ftp:|^data:image\/[a-z0-9+-]+;base64,[a-z0-9+\/]+=*$|^blob:/i;
