...
soy.examples.features.demoDoubleBraces = function(opt_data, opt_ignored, opt_ijData) {
  return soydata.markUnsanitizedText('The set of ' + ('' + opt_data.setName) + ' is {' + soy.examples.features.buildCommaSeparatedList_({items: opt_data.setMembers}, null, opt_ijData) + ', ...}.');
};
if (goog.DEBUG) {
...
soy.examples.features.demoBidiSupport = function(opt_data, opt_ignored, opt_ijData) {
  ...
  output += '</select></div><a href="#" style="' + ((opt_ijData.csp_nonce) ? '/*' + opt_ijData.csp_nonce + '*/' : '') + 'float:' + soy.$$escapeHtmlAttribute(soy.$$filterCssValue('right')) + '">Help</a><br>';
  return soydata.VERY_UNSAFE.ordainSanitizedHtml(output);
};
if (goog.DEBUG) {
